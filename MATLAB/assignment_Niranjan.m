...
axis equal 
xlim([0 12])
xticks([0 2 4 6 8 10 12]) %labelling x-axis or K_t grid intervals                                          
ylim([0,12])
...
hold on
plot (K_t(15), total_savings(15),'r*'); %To label the point where total savings graph cuts total depreciation
hold off 
legend('Total Savings','Total Depreciation','Total Output', 'Total Savings equals TotalDepreciation', 'Location', 'southeast');
...
Y = sprintf('%s is steady-state output and %d steady-state consumption for initial capital value 1.\n',Yss2,Css2);
...
Css3 = (1-s)*Yss3; 
%Answer - No,the answer does not change when we change intial value of capital to 12. In Workspace we can see 
%that value of TKss2 and TKss3 are precisely equal (round off) also steady-state output and
%steady-state consumption are equal. This also tells us that steady-state is
%globally stable so irrespective of initial point(K_t>0) in the long run we
%will move to steady-state. 

X = sprintf('%s is steady-state output and %d steady-state consumption for initial cpaital guess 12.\n',Yss3,Css3); 
% To display steady-state output and steady-state consumption together
disp(X);
